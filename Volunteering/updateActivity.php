...
?>
<form action="redirect.php" method="post" name="activity" >
...
<legend><h3>Volunteer Activity Update Form</h3></legend>
 ...
 <tr><td><label for="fromDate">From Date *</label></td>
	  ...
	  <option value="NULL">--Select--</option>
          <?php  
			$result1 = "SELECT name FROM project_partners";
		  ...
		  while($row1 = mysql_fetch_array($projectResult1))
			...
			<option value="<?php echo "".$partnerid; ?>" name="<?php  echo "".$projectid1; ?>"><?php echo "".$name1; ?></option>
			<?php } ?>
      </select>
...
<?php if(isset($_POST['submit'])) 
	if($_POST['formname']=="updateActivity")
	{
		...
		$fromDate=date_format(date_create_from_format('j-M-Y',$_POST['fromDate']),'Y-m-d');
		$toDate=date_format(date_create_from_format('j-M-Y',$_POST['fromDate']),'Y-m-d');

		//echo "<script>alert(".$fromDate.");</script>";	
		
		$volunteerInsertAtts= "donor_id,from_date,to_date,from_time,to_time,hours,donation_type,area,activity_done,onsite_offsite,location,organisation,notes,project_id,approval_status,partner_id";
	
			$volunteerValues="$donorid,'".$fromDate."','".$toDate."','".$_POST['fromTime']."','".$_POST['toTime']."','".$_POST['hours']."','$type','".$_POST['area']."','".$_POST['activity']."','".$_POST['on_off']."','".$_POST['location']."','".$_POST['organisation']."','notesss','".$_POST['project']."','P'";
