...
?>
<h3 align="left"> <?php echo $resultCount; ?> requests expired.</h4>
<hr>
<table align="center" class="table-div2">
	<?php
		while($row = mysql_fetch_array($result))
		{
			...
			$opp_query="SELECT * FROM volunteering_opportunities WHERE approval_status='A' AND to_date<CURDATE() AND activity_id=".$row['activity_id']; 
...
<?php } ?>
	...
	$(".opportunity input[type='text']").on('change',function(){
		...
		$("#add_opp<?php echo $row['activity_id'];?>").click(function(){
			...
			var place="<td><select name='place' id='place' style='max-width:100px;'><option value=''>Select</option><?php echo $options;?></select></td>";
