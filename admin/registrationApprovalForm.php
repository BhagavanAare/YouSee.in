 ...
 while($row = mysql_fetch_array($ngoResult))
  {
	...
	<td><?php echo $count++; ?></td>
    <td><?php echo "".$row[$user['username']];?></td>
	<input type="hidden" name=<?php echo "".$user['username']."".$userid; ?> value=<?php echo "".$row[$user['username']];?>
...
if (isset($_POST['donorSubmit']) || isset($_POST['ngoSubmit'])) 
{
	...
	$counter=0;
	 while($count)
	 {
		 ...
		 $usernameHidden="".$user['username']."".$userid."";
		 $usename=$_POST[$usernameHidden];
...
{
	global $user;
	echo "SELECT * FROM donors SET WHERE ".$user['id']."='".$userID."'<br />";
	$dp = mysql_query("SELECT * FROM donors SET WHERE ".$user['id']."='".$userID."''");
	echo $dp;
